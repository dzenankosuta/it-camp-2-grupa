...
// After you did the job with new array,
//you need to sum all prices of every product and return total sum value, and add a FIXED 320 shipping costs.
...
function funkcija(obj) {
  const newObj = obj.map((el) => {
    return {
      ime: el.name,
      cena: function () {
        const nova = obj.reduce((acc, cur) => acc + cur.amount * cur.price, 0);
        return nova;
      },
    };
  });
  const broj = newObj.length;
  const sveCene = newObj.reduce((acc, cur) => acc + cur.cena(), 0);
  return `${sveCene} ovo su cene a zajedno sa postarinom su ${
    sveCene + 320
  } a broj je ${broj}`;
}
