...
function products(arr) {
  ...
  const allPrice = newArr.map((element) => element.totalprice);
  const suma = allPrice.reduce(
    (previousValue, currentValue) => previousValue + currentValue
  );
  const total = suma + shippingCost;
  return `You got ${arr.length} in cart, shipping cost is ${shippingCost}, your total is ${total}.`;
}
console.log(products(CART_DATA));
